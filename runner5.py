...
def main():
    ...
    screen_width = WIDTH * CELL_SIZE + 100
    screen_height = HEIGHT * CELL_SIZE + 150
    screen = pygame.display.set_mode((screen_width, screen_height))
    pygame.display.set_caption("AI Police - Hot & Cold Radar (Upgraded UI)")
    ...
    assets = {}
    try:
        if os.path.exists("police_bg.jpg"):
            bg = pygame.image.load("police_bg.jpg")
            assets['bg'] = pygame.transform.scale(bg, (screen_width, screen_height))
        if os.path.exists("thief.png"):
            thief_img = pygame.image.load("thief.png")
            assets['thief'] = pygame.transform.scale(thief_img, (CELL_SIZE, CELL_SIZE))
        if os.path.exists("police.png"):
            police_img = pygame.image.load("police.png")
            assets['police'] = pygame.transform.scale(police_img, (CELL_SIZE, CELL_SIZE))
        if os.path.exists("jail.png"):
            jail_img = pygame.image.load("jail.png")
            assets['jail'] = pygame.transform.scale(jail_img, (CELL_SIZE, CELL_SIZE))
        if os.path.exists("background_siren.wav"):
            pygame.mixer.music.load("background_siren.wav")
            pygame.mixer.music.play(-1) # Loop indefinitely
            pygame.mixer.music.set_volume(0.3)
    except Exception as e:
        print("Warning: Could not load all assets:", e)
        ...
        for r in range(HEIGHT):
            for c in range(WIDTH):
                ...
                if setup_mode:
                    ...
                    if (r,c) in ai.mines: 
                        color = THIEF_RED
                        img_to_draw = assets.get('jail') # BUSTED! Put in jail
                        if not img_to_draw:
                            text = "BUSTED"
                            txt_col = WHITE
                    elif (r,c) in ai.safes and (r,c) not in revealed:
                        color = POLICE_BLUE 
                        img_to_draw = assets.get('police') # AI marked safe, planning to clear
