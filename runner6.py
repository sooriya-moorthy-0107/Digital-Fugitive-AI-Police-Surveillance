...
def main():
    ...
    # 1. Safely load images individually
    image_files = [
        ("police_bg.jpg", "bg"), 
        ("thief.png", "thief"), 
        ("police.png", "police"), 
    ...
    for filename, key in image_files:
        try:
            if os.path.exists(filename):
                ...
                if key == "bg":
                    assets[key] = pygame.transform.scale(img, (screen_width, screen_height))
                else:
                    assets[key] = pygame.transform.scale(img, (CELL_SIZE, CELL_SIZE))
        except Exception as e:
            print(f"Warning: Could not load image '{filename}': {e}")

    # 2. Safely load and play audio separately
    try:
        if os.path.exists("background_siren.wav"):
            pygame.mixer.music.load("background_siren.wav")
            pygame.mixer.music.set_volume(0.6) # Increased volume to 0.6
            pygame.mixer.music.play(-1) # Loop indefinitely
            print("Audio: Siren background music started successfully!")
        else:
            print("Audio: 'background_siren.wav' not found in the folder.")
    except Exception as e:
